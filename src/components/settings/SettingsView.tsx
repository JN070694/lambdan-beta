...
{`3,Explain supply and demand.,,,,,,The law of supply and demand describes how price and quantity interact in a market.,,Economics,`}
            ...
            <p style={{ marginBottom: 0 }}>
              ...
              quiz: it links an image to one or more specific questions, e.g. a source that
              specifies "use this chart for questions 5–11." Save the chart as an image, give
              it a unique name (<code>n5</code>), then apply that same nid to every question in
              the range — one nid can cover as many questions as needed. Because nid matching
              applies across the whole pack, reusing a nid in a second quiz within the same
              tar.gz will incorrectly attach that image there as well; a unique-per-quiz nid
              avoids this. For multiple images in one range, append a trailing letter
              (<code>n5</code>, <code>n5a</code>, <code>n5b</code>, …) — LAMBDAn groups these
              automatically. Leave nid blank if a question has no image. LAMBDAn does not
              generate these images — save the chart or figure yourself and add it to your
              pack's <strong>media</strong> folder.
            </p>
          </div>
        </div>
      )}
