...
{`3,Explain supply and demand.,,,,,,The law of supply and demand describes how price and quantity interact in a market.,,Economics,`}
            ...
            <p style={{ fontWeight: 700, marginBottom: 4, marginTop: 8 }}>Explanation</p>
            <p style={{ marginBottom: 0 }}>
              Optional, but common — a short paragraph or formula showing how to arrive
              at the answer. When filled in, it appears beneath the correct/incorrect feedback
              after you answer a question during a quiz.
            </p>

            <p style={{ fontWeight: 700, marginBottom: 4, marginTop: 14 }}>Group</p>
            <p style={{ marginBottom: 0 }}>
              Optional field to categorize recurring types of questions. If the source you're
              importing breaks its questions into areas of discipline within a given subject,
              those labels should be included here for tracking knowledge of a particular
              topical subject. If the provided source material doesn't cite distinct areas of
              discipline, group by question types instead. Leave blank if there's nothing
              meaningful to group by.
            </p>

            <p style={{ fontWeight: 700, marginBottom: 4, marginTop: 14 }}>Nid</p>
            <p style={{ marginBottom: 0 }}>
              Optional, and intentionally the last field — set and tracked by the user rather
              than generated automatically. Reference images ([bracket] filenames) are shared
              across every quiz in a tar.gz pack, similar to a tax bracket table every tax quiz
              might need. The <code>nid</code> is narrower conceptually and scoped to a single
              quiz: it links an image or images to one or more specific questions, e.g. a source
              that specifies "use this chart for questions 11–17." Save the media as an image,
              give it a unique name (<code>n11</code>), then apply that same nid value or range
              to every question in the range. Because nid matching applies across a whole pack,
              reusing a nid in a second quiz within the same tar.gz will incorrectly attach that
              image there as well whereas a unique-per-quiz nid avoids this. For multiple images
              in a range, combine the nids by appending a trailing letter and the + symbol
              (<code>n11+n11a+n11b</code>, …) — LAMBDAn groups these automatically. Leave nid
              blank if a question has no image. LAMBDAn does not generate these images — save
              the chart or figure yourself and add it to your pack's <strong>media</strong> folder.
            </p>
          </div>
        </div>
      )}
