...
{`3,Explain supply and demand.,,,,,,The law of supply and demand describes how price and quantity interact in a market.,,Economics,`}
            ...
            <p style={{ marginBottom: 0 }}>
              Optional, and intentionally the last field — set and tracked by hand rather than
              generated automatically. Reference images ([bracket] filenames) are quiz-wide,
              like a tax bracket table every tax question might need. Nid is narrower: it links
              an image to specific questions, e.g. a source that says "use this chart for
              questions 5–11." Save the chart as an image, name it after the first question
              that needs it (<code>n5</code>), then set that same nid on every question in the
              range — one nid can cover as many questions as needed. For multiple images in
              one range, add a trailing letter (<code>n5</code>, <code>n5a</code>, <code>n5b</code>,
              …); LAMBDAn groups them automatically. Leave Nid blank if a question has no
              image. Images aren't generated by LAMBDAn — save the chart or figure yourself
              and add it to your pack's <strong>media</strong> folder.
            </p>
          </div>
        </div>
      )}
